...
export class HomePage {
    ...
    UPCOMING_USER_EVENTS: string = "http://campus.us-west-2.elasticbeanstalk.com/user_upcoming_events/" + this.user_id + "/";
    ...
    RATE_USER_URL: string = "http://campus.us-west-2.elasticbeanstalk.com/rate_user/"
    user_ratings: any;
    interests: any;
    user_interests: any;
    profile_image: any;
    ...
    showRadio(owner, type) {
        let alert = this.alertCtrl.create();
        alert.setTitle('Please select your rating');
        ...
        alert.addButton({
            ...
            handler: data => {
                ...
                this.rateUser(owner, type, +this.testRadioResult);
            }
        });
        alert.present();
    }

    rateUser(owner, type, rating) {
        var params = {
        ...
        this.http.post(this.RATE_USER_URL, JSON.stringify(params), { headers: this.contentHeader })
            .map(res => res.json())
            .subscribe(
            data => {
                console.log("your rate is added!!")
    ...
    getUpcomingEvents() {
        this.http.get(this.UPCOMING_USER_EVENTS, { headers: this.contentHeader })
            .map(res => res.json())
            .subscribe(
            data => {
    ...
    pushMessages() {
        this.navCtrl.push(MessageboxPage, { animate: true, direction: 'back' })
    }
